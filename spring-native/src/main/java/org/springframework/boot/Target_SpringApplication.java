...
final class Target_SpringApplication {
	...
	public Target_SpringApplication(ResourceLoader resourceLoader, Class<?>... primarySources) {
		...
		this.primarySources = SpringAotApplication.SPRING_AOT ?
		...
		if (SpringAotApplication.SPRING_AOT) {
			logger.info("AOT mode enabled");
			setApplicationContextFactory(SpringAotApplication.AOT_FACTORY);
			setInitializers(Arrays.asList(SpringAotApplication.getBootstrapInitializer(), new ConditionEvaluationReportLoggingListener()));
		}
		else {
